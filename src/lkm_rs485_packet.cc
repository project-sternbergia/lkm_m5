...
RequestPacket::RequestPacket(uint8_t type, uint8_t id, uint8_t data_len)
: Packet(type, id, data_len)
{
}
...
bool RequestPacket::pack()
{
  ...
  frame_[CMD_FRAME_CHECK_BYTE] =
    calc_checksum(frame_.begin(), frame_.begin() + CMD_FRAME_LENGTH - 1);
  if (data_length() > 0) {
    frame_.back() = calc_checksum(
      frame_.begin() + CMD_FRAME_LENGTH, frame_.begin() + CMD_FRAME_LENGTH + data_length());
  }
  return true;
}
...
bool ResponsePacket::unpack()
{
  ...
  if (data_length() > 0) {
    ...
    if (frame_.size() != CMD_FRAME_LENGTH + data_length() + 1) {
      LKM_DEBUG_PRINTF(
        "Invalid received data length. except=[%d] actual=[%u]\n",
        CMD_FRAME_LENGTH + data_length() + 1, frame_.size());
      return false;
    }

    // check data frame parity
    uint8_t data_chk = calc_checksum(
      frame_.begin() + CMD_FRAME_LENGTH, frame_.begin() + CMD_FRAME_LENGTH + data_length());
    if (data_chk != frame_.back()) {
      LKM_DEBUG_PRINTF(
        "Invalid data frame checksum. except=[%x] actual=[%x]\n", data_chk, frame_.back());
      return false;
    }
  }
...
bool MotorStateResponsePacket::unpack()
{
  ...
  uint8_t * data_head = frame_.data() + CMD_FRAME_LENGTH;
  tempareture_ = *data_head;

  // set torque current
  int16_t current = 0.0f;
  std::memcpy(&current, data_head + 1, sizeof(current));
  ...
  // set speed
  int16_t speed_dps = 0;
  std::memcpy(&speed_dps, data_head + 3, sizeof(speed_dps));
  speed_ = speed_dps / 180.0f * M_PI;

  // set position
  uint16_t position_step = 0;
  std::memcpy(&position_step, data_head + 5, sizeof(position_step));
...
: ResponsePacket(CMD_READ_ENCODER, 6, frame), encoder_type_(encoder_type)
{
}
...
bool ReadEncoderResponsePacket::unpack()
{
  ...
  std::memcpy(&position_step, frame_.data() + CMD_FRAME_LENGTH, sizeof(position_step));

  uint16_t raw_position_step;
  std::memcpy(&raw_position_step, frame_.data() + CMD_FRAME_LENGTH + 2, sizeof(raw_position_step));

  uint16_t offset;
  std::memcpy(&offset, frame_.data() + CMD_FRAME_LENGTH + 4, sizeof(offset));
...
{
  power_control_ = std::max(
    std::min(power_control, static_cast<int16_t>(MS_SERIES_MAX_POWER)),
    static_cast<int16_t>(-MS_SERIES_MAX_POWER));
}

bool OpenLoopControlRequestPacket::pack()
{
  if (!RequestPacket::pack()) return false;
  std::memcpy(frame_.data() + CMD_FRAME_LENGTH, &power_control_, sizeof(power_control_));
  return true;
}

TorqueClosedLoopControlRequestPacket::TorqueClosedLoopControlRequestPacket(
  uint8_t id, float iq_control, uint8_t motor_type)
: RequestPacket(CMD_TORQUE_CLOSED_LOOP_CONTROL, id, 2), motor_type_(motor_type)
{
...
: RequestPacket(CMD_SPEED_CLOSED_LOOP_CONTROL, id, 4), speed_(speed)
{
}
...
bool SpeedClosedLoopControlRequestPacket::pack()
{
  ...
  std::memcpy(frame_.data() + CMD_FRAME_LENGTH, &speed_dps, sizeof(speed_dps));
  return RequestPacket::pack();
}

MultiLoopAngleControlRequestPacket::MultiLoopAngleControlRequestPacket(uint8_t id, float angle)
: RequestPacket(CMD_MULTI_LOOP_ANGLE_CONTROL_1, id, 8), angle_(angle)
{
}

bool MultiLoopAngleControlRequestPacket::pack()
{
  int64_t angle_deg = static_cast<int64_t>(angle_ * 180.0f / M_PI * 100.0);
  std::memcpy(frame_.data() + CMD_FRAME_LENGTH, &angle_deg, sizeof(angle_deg));
  return RequestPacket::pack();
}

MultiLoopAngleControlWithSpeedRequestPacket::MultiLoopAngleControlWithSpeedRequestPacket(
  uint8_t id, float angle, float speed)
: RequestPacket(CMD_MULTI_LOOP_ANGLE_CONTROL_2, id, 12), angle_(angle), speed_(speed)
{
}

bool MultiLoopAngleControlWithSpeedRequestPacket::pack()
{
  int32_t angle_deg = static_cast<int64_t>(angle_ * 180.0f / M_PI * 100.0);
  std::memcpy(frame_.data() + CMD_FRAME_LENGTH, &angle_deg, sizeof(angle_deg));

  uint32_t speed_dps = static_cast<uint32_t>(std::abs(speed_) * 180.0f / M_PI * 100.0);
  std::memcpy(frame_.data() + CMD_FRAME_LENGTH + 8, &speed_dps, sizeof(speed_dps));
  return RequestPacket::pack();
}

SingleLoopAngleControlRequestPacket::SingleLoopAngleControlRequestPacket(
  uint8_t id, uint8_t spin_dir, float angle)
: RequestPacket(CMD_SINGLE_LOOP_ANGLE_CONTROL_1, id, 4), spin_dir_(spin_dir), angle_(angle)
{
}

bool SingleLoopAngleControlRequestPacket::pack()
{
  frame_[CMD_FRAME_LENGTH] = spin_dir_;
  uint16_t angle_deg = static_cast<uint16_t>(angle_ * 180.0f / M_PI * 100.0);
  std::memcpy(frame_.data() + CMD_FRAME_LENGTH + 1, &angle_deg, sizeof(angle_deg));
  return RequestPacket::pack();
}

SingleLoopAngleControlWithSpeedRequestPacket::SingleLoopAngleControlWithSpeedRequestPacket(
...
{
  frame_[CMD_FRAME_LENGTH] = spin_dir_;
  uint16_t angle_deg = static_cast<uint16_t>(angle_ * 180.0f / M_PI * 100.0f);
  std::memcpy(frame_.data() + CMD_FRAME_LENGTH + 1, &angle_deg, sizeof(angle_deg));

  uint32_t speed_dps = static_cast<uint32_t>(speed_ * 180.0f / M_PI);
  std::memcpy(frame_.data() + CMD_FRAME_LENGTH + 4, &speed_dps, sizeof(speed_dps));
  return RequestPacket::pack();
}

IncrementAngleControlRequestPacket::IncrementAngleControlRequestPacket(uint8_t id, float angle)
: RequestPacket(CMD_INCREMENT_ANGLE_CONTROL_1, id, 4), angle_(angle)
{
}
...
IncrementAngleControlWithSpeedRequestPacket::IncrementAngleControlWithSpeedRequestPacket(
  uint8_t id, float angle, float speed)
: RequestPacket(CMD_INCREMENT_ANGLE_CONTROL_2, id, 8), angle_(angle), speed_(speed)
{
}
...
bool IncrementAngleControlWithSpeedRequestPacket::pack()
{
  ...
  uint32_t speed_dps = static_cast<uint16_t>(speed_ * 180.0f / M_PI);
  std::memcpy(frame_.data() + CMD_FRAME_LENGTH + 4, &speed_dps, sizeof(speed_dps));

  return RequestPacket::pack();
}

WritePIDParameterToRAMRequestPacket::WritePIDParameterToRAMRequestPacket(
  uint8_t id, uint8_t angle_kp, uint8_t angle_ki, uint8_t speed_kp, uint8_t speed_ki, uint8_t iq_kp,
  uint8_t iq_ki)
...
WritePIDParameterToROMRequestPacket::WritePIDParameterToROMRequestPacket(
  uint8_t id, uint8_t angle_kp, uint8_t angle_ki, uint8_t speed_kp, uint8_t speed_ki, uint8_t iq_kp,
  uint8_t iq_ki)
...
: RequestPacket(CMD_WRITE_ACCELERATION, id, 8), accerelation_(acc)
{
}

WriteEncoderValueToROMAsZeroPointRequestPacket::WriteEncoderValueToROMAsZeroPointRequestPacket(
  uint8_t id, uint16_t encoder_offset)
: RequestPacket(CMD_WRITE_ACCELERATION, id, 8), encoder_offset_(encoder_offset)
{
}
