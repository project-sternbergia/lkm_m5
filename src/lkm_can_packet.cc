...
: ResponsePacket(CMD_READ_ENCODER, frame), encoder_type_(encoder_type)
{
}
...
TorqueClosedLoopControlRequestPacket::TorqueClosedLoopControlRequestPacket(
  float iq_control, uint8_t motor_type)
: RequestPacket(CMD_TORQUE_CLOSED_LOOP_CONTROL, 8), motor_type_(motor_type)
{
...
: RequestPacket(CMD_SPEED_CLOSED_LOOP_CONTROL, 8), speed_(speed)
{
}
...
: RequestPacket(CMD_MULTI_LOOP_ANGLE_CONTROL_1, 8), angle_(angle)
{
}
...
: RequestPacket(CMD_MULTI_LOOP_ANGLE_CONTROL_2, 8), angle_(angle), speed_(speed)
{
}
...
{
  if (!RequestPacket::pack()) return false;
  uint16_t speed_dps = static_cast<uint16_t>(std::abs(speed_) * 180.0f / M_PI);
...
SingleLoopAngleControlRequestPacket::SingleLoopAngleControlRequestPacket(
  uint8_t spin_dir, float angle)
: RequestPacket(CMD_SINGLE_LOOP_ANGLE_CONTROL_1, 8), spin_dir_(spin_dir), angle_(angle)
{
}
...
SingleLoopAngleControlWithSpeedRequestPacket::SingleLoopAngleControlWithSpeedRequestPacket(
  uint8_t spin_dir, float angle, float speed)
...
: RequestPacket(CMD_INCREMENT_ANGLE_CONTROL_1, 8), angle_(angle)
{
}
...
IncrementAngleControlWithSpeedRequestPacket::IncrementAngleControlWithSpeedRequestPacket(
  float angle, float speed)
: RequestPacket(CMD_INCREMENT_ANGLE_CONTROL_2, 8), angle_(angle), speed_(speed)
{
}
...
: RequestPacket(CMD_WRITE_ACCELERATION, 8), accerelation_(acc)
{
}

WriteEncoderValueToROMAsZeroPointRequestPacket::WriteEncoderValueToROMAsZeroPointRequestPacket(
  uint16_t encoder_offset)
: RequestPacket(CMD_WRITE_ACCELERATION, 8), encoder_offset_(encoder_offset)
{
}
