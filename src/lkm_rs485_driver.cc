...
: lkm_m5::Driver(master_id, target_id, motor_type, encoder_type), send_count_(0)
{
}
...
{
  LKM_DEBUG_FUNC
  LKM_DEBUG_PRINTF("%s is not implemented.", __func__);
...
bool Driver::write_current_position_to_rom_as_zero_point()
{
  LKM_DEBUG_FUNC
  auto packet =
    RequestPacket(CMD_WRITE_CURRENT_POSITION_TO_RON_AS_THE_MOTOR_ZERO_POINT, target_id(), 0);
  if (!packet.pack()) return false;
  return send_motor_request(packet.frame());
}
...
bool Driver::send_motor_request(const Frame & packet)
{
  ...
  LKM_DEBUG_PRINTF("Send data %d : ", packet.size());
  for (uint8_t idx = 0; idx < packet.size(); ++idx) {
    LKM_DEBUG_PRINTF("%02x ", packet[idx]);
  }
  LKM_DEBUG_PRINTLN("");
...
bool Driver::process_packet()
{
  ...
  while (stream_->available()) {
    receive_buffer_.push(stream_->read());
    ++cnt;
  }

  while (!receive_buffer_.isEmpty() && receive_buffer_[0] != CMD_FRAME_HEADER) {
    uint8_t dat;
    receive_buffer_.pop(dat);
    LKM_DEBUG_PRINTF("Dispose unused packet %x", dat);
  }

  if (receive_buffer_.size() < CMD_FRAME_LENGTH) {
    return false;
  }

  uint16_t data_length = CMD_FRAME_LENGTH;
  if (receive_buffer_[CMD_FRAME_DATA_LENGTH_BYTE] > 0)
    data_length += receive_buffer_[CMD_FRAME_DATA_LENGTH_BYTE] + 1;

  LKM_DEBUG_PRINTF("Receive data %d : ", data_length);
  Frame receive_data(data_length);
  for (uint16_t idx = 0; idx < data_length; ++idx) {
    receive_buffer_.pop(receive_data[idx]);
    LKM_DEBUG_PRINTF("%02x ", receive_data[idx]);
  }
  LKM_DEBUG_PRINTLN("");
...
bool Driver::process_response_packet(const Frame & frame)
{
  ...
  if (
    ...
    LKM_DEBUG_PRINTLN("Receive MotorStateResponsePacket");
